...
export const authRepository = {
    ...
    async fnGetTwoFactorSecret(tx: TxClient, userId: string): Promise<string | null> {
        const result = await tx.query("SELECT auth.fn_get_two_factor_secret($1) AS secret", [
            userId,
        ]);
        return (result.rows[0]?.secret as string | null) ?? null;
    },

    spConsumeRecoveryCode(
        tx: TxClient,
        userId: string,
        codeHash: string,
    ): Promise<{ ok: boolean }> {
        return callResult(tx, "CALL auth.sp_consume_recovery_code($1, $2, NULL)", [
            userId,
            codeHash,
        ]);
    },
